...
from forms import DataSourceForm  # Import the form you created
...
app = Flask(__name__)
app.secret_key = 'development'  # Change this to a random secret key
...
def homepage():
    ...
    if user_info:
        ...
        return render_template('dashboard.html', user_info=user_info, groups=groups, user_role=user_role)
...
def authorized():
    ...
    user_id = user_info.get('id')  # Adjust if the ID is under a different key in the response
    user_name = user_info.get('displayName')  # Or appropriate field for the user's name
    user_email = user_info.get('userPrincipalName')  # Or appropriate field for the user's email
    ...
    session['user_role'] = 'admin' if is_admin else 'user'  # It seems there was a typo here, setting 'user' for non-admins
...
def create_data_source():
    ...
    if form.validate_on_submit():
        ...
        data_source = DataSource(
            ...
            aws_resource_arn=form.aws_resource_arn.data,  # This field is for the AWS ARN
        ...
        # Here, you should trigger the AAD group creation and store the returned group ID
        # For now, we'll simulate this with a placeholder
        # data_source.aad_group_id = "dummy-aad-group-id"  # Placeholder for the AAD group ID
        # db.session.commit()

        flash('Data source created successfully!', 'success')
        return redirect(url_for('list_data_sources'))  # Redirect to the homepage or list of data sources
...
def list_data_sources():
    ...
    for data_source in data_sources:
        info = {
            ...
            'created_by': User.query.get(data_source.created_by).name if data_source.created_by else 'N/A',  # Assuming 'created_by' is a field in your DataSource model
            'created_at': data_source.created_at.strftime('%Y-%m-%d %H:%M:%S')  # Format the date as you prefer
        }
...
def check_if_user_is_admin(group_ids):
 ...
 # Extracting the 'displayName' from each group and checking if 'admin_group_name' is one of them
    is_user_admin = any(group.get('displayName') == admin_group_name for group in group_ids)
