...
def init_app(app):
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///data_access_manager.db'  # Ensure this is the correct path
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # Optional: Disable event system if not needed
    db.init_app(app)

    with app.app_context():
        db.create_all()  # This command should create the database file
...
class DataSource(db.Model):
    ...
    aws_resource_arn = db.Column(db.String(255))  # If applicable, ARN for the corresponding AWS resource
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    created_by = db.Column(db.String(255), db.ForeignKey('users.id'))

    # Relationships
    permissions = db.relationship('UserDataSourcePermission', backref='data_source', lazy=True)
...
class User(db.Model):
    ...
    permissions = db.relationship('UserDataSourcePermission', backref='user_info_backref', lazy=True)
    def __repr__(self):
